...
不同的sampler会输出不同的内容，即使在生成中使用了相同的prompts和step，
有的比较相似有的会大不同。
一般推荐使用 `k_euler_ancestral`，因为其能够提供相对稳定的高质量图片生成
(欢迎在 issue 中讨论各种sampler的区别)。

### 调整影响因子

使用半角方括号 `[]` 包裹关键词以弱化该关键词的权重，使用半角花括号 `{}` 包裹关键词以强化该关键词的影响。例如：

```
约稿 [tears], {spread legs}
```

可以通过多次使用括号来进一步增强或减弱关键词的权重，每一层括号会增强/减弱1.05倍的权重。例如：
...
这时可以在关键词后添加`:x`来指定单个关键词的权重，`x`的取值范围是`0.1~100`，默认为1。举例：
